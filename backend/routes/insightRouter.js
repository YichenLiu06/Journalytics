...
preamble = "You will be asked to give specific, personalized insights/advice based on other's diary/journal entries. You are speaking directly to the author of these entries. Keep your responses concise. Use a conversational and enthusiastic tone."
...
insightRouter.use(express.urlencoded({ extended: true }));
insightRouter.use(express.json());

insightRouter.post("", passport.authenticate('jwt', { session: false }), async (req, res) => {
  res.json({
    sentiment: (await analyzeSentiment(req.body.content))[0].prediction,
    analysis: (await analyzeChat(req.body.content)).text
  })
})
